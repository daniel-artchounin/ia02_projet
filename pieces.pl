...
insertNewSbire(C, h, X, Y) :- 	write('* Insertion d\'un nouveau sbire *'), hSep, nl,
								repeat,
...
changePiecePosition(XOld, YOld, XNew, YNew, r) :-	ocreKalista(XNew, YNew), % The ocre player has lost the game	
...
changePiecePosition(XOld, YOld, XNew, YNew, o) :- 	redKalista(XNew, YNew), % The red player has lost the game
...
changePositionOrNewSbire(C, m) :- 	possibleMoves(Moves, 1, C, _),
									otherPlayer(C, C2), 
									kalista(XK, YK, C2),
									element((_, _, XK, YK), Moves),									
									managePositionOrNewSbire(1, C, m), % Eat that Kalista !
									!.
changePositionOrNewSbire(C, m) :- 	managePositionOrNewSbire(2, C, m), % Try to insert a sbire such as we can eat that Kalista !
									!.
changePositionOrNewSbire(C, m) :- 	managePositionOrNewSbire(1, C, m), % Well, just move a piece at best place.
!.
...
managePositionOrNewSbire(2, C, T) :- 	insertNewSbire(C, T, _, _),
										!.
...
getPiecesOnType(Pieces, C, P) :- setof(
									(X, Y),
									pieceOn(X, Y, P, C), 
									Pieces
								),
...
% To make a list of all specific colored pieces on a type of place (1, 2 or 3).
% ===> That special formatting should be use only for the "possibleMoves" predicate !
